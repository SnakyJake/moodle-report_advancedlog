 ...
 * @return void
 */
function report_advancedlog_print_graph($course, $user, $typeormode, $date=0, $logreader='') {
    global $CFG, $OUTPUT;
    ...
    if ($typeormode == 'today' || $typeormode == 'userday.png') {
        $logs = report_advancedlog_usertoday_data($course, $user, $date, $logreader);
        $title = get_string("hitsoncoursetoday", "", $a);
    } else if ($typeormode == 'all' || $typeormode == 'usercourse.png') {
        $logs = report_advancedlog_userall_data($course, $user, $logreader);
 ...
 * @return array
 */
function report_advancedlog_usercourse($userid, $courseid, $coursestart, $logreader = '') {
    global $DB;
 ...
 * @return array
 */
function report_advancedlog_userday($userid, $courseid, $daystart, $logreader = '') {
    global $DB;
    $logmanager = get_log_manager();
    $readers = $logmanager->get_readers();
    if (empty($logreader)) {
 ...
 * @return void
 */
function report_advancedlog_print_mnet_selector_form($hostid, $course, $selecteduser=0, $selecteddate='today',
    ...
    foreach($hostarray as $hostid => $name) {
        ...
        if ($CFG->mnet_localhost_id == $hostid) {
            if (has_capability('report/advancedlog:view', $sitecontext) && $showcourses) {
    ...
    if (has_capability('report/advancedlog:view', $sitecontext) && !$course->category) {
    ...
    echo "<form class=\"logselectform\" action=\"$CFG->wwwroot/report/advancedlog/index.php\" method=\"get\">\n";
    ...
    if (has_capability('report/advancedlog:view', $sitecontext) && $showcourses) {
        ...
        if (has_capability('report/advancedlog:view', $sitecontext)) {
            $a = new stdClass();
            $a->url = "$CFG->wwwroot/report/advancedlog/index.php?chooselog=0&group=$selectedgroup&user=$selecteduser"
    ...
    echo html_writer::label(get_string('logsformat', 'report_advancedlog'), 'menulogformat', false, array('class' => 'accesshide'));
 ...
 */
function report_advancedlog_userall_data($course, $user, $logreader) {
    global $CFG;
    $site = get_site();
    $timenow = time();
    $logs = [];
    ...
    $rawlogs = report_advancedlog_usercourse($user->id, $courseselect, $coursestart, $logreader);
 ...
 */
function report_advancedlog_usertoday_data($course, $user, $date, $logreader) {
    $site = get_site();
    $logs = [];
    ...
    $rawlogs = report_advancedlog_userday($user->id, $courseselect, $daystart, $logreader);
