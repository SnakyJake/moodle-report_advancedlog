...
require_once($CFG->dirroot.'/report/advancedlog/locallib.php');
...
$url = new moodle_url("/report/advancedlog/index.php", $params);

$PAGE->set_url('/report/advancedlog/index.php', array('id' => $id));
...
require_capability('report/advancedlog:view', $context);

// When user choose to view logs then only trigger event.
if ($chooselog) {
    // Trigger a report viewed event.
    $event = \report_advancedlog\event\report_viewed::create(array('context' => $context, 'relateduserid' => $user,
...
if (!empty($page)) {
    $strlogs = get_string('logs'). ": ". get_string('page', 'report_advancedlog', $page + 1);
} else {
...
if (empty($course) || ($course->id == $SITE->id)) {
    admin_externalpage_setup('reportadvancedlog', '', null, '', array('pagelayout' => 'report'));
...
$reportlog = new report_advancedlog_renderable($logreader, $course, $user, $modid, $modaction, $group, $edulevel, $showcourses, $showusers,
        $chooselog, true, $url, $date, $logformat, $page, $perpage, 'timecreated DESC', $origin);
$readers = $reportlog->get_readers();
$output = $PAGE->get_renderer('report_advancedlog');

if (empty($readers)) {
    echo $output->header();
    echo $output->heading(get_string('nologreaderenabled', 'report_advancedlog'));
} else {
    if (!empty($chooselog)) {
        ...
        if (empty($logformat)) {
            ...
            $pluginname = get_string('pluginname', 'report_advancedlog');
