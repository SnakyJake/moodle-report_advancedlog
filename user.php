...
if (!$today && !$all) {
    print_error('nocapability', 'report_advancedlog');
}

if ($mode === 'today') {
    if (!$today) {
        require_capability('report/advancedlog:viewtoday', $coursecontext);
    }
} else {
    if (!$all) {
        require_capability('report/advancedlog:view', $coursecontext);
    }
}
...
$PAGE->set_url('/report/advancedlog/user.php', array('id' => $user->id, 'course' => $course->id, 'mode' => $mode));
...
$event = \report_advancedlog\event\user_report_viewed::create(array('context' => $coursecontext, 'relateduserid' => $userid,
...
$reportlog = new report_advancedlog_renderable($logreader, $course, $user->id, 0, '', -1, -1, false, false, true, false, $PAGE->url,
...
report_advancedlog_print_graph($course, $user, $mode, 0, $logreader);
echo '</div>';
