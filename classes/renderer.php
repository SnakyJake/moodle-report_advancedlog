 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class report_advancedlog_renderer extends plugin_renderer_base {

    /**
     * This method should never be manually called, it should only be called by process.
     *
     * @deprecated since 2.8, to be removed in 2.9
     * @param report_advancedlog_renderable $reportlog
     * @return string
     */
    public function render_report_advancedlog_renderable(report_advancedlog_renderable $reportlog) {
     ...
     *
     * @param report_advancedlog_renderable $reportlog object of report_advancedlog.
     */
    protected function render_report_advancedlog(report_advancedlog_renderable $reportlog) {
        if (empty($reportlog->selectedlogreader)) {
            echo $this->output->notification(get_string('nologreaderenabled', 'report_advancedlog'), 'notifyproblem');
            return;
        }
     ...
     *
     * @param report_advancedlog_renderable $reportlog log report.
     */
    public function reader_selector(report_advancedlog_renderable $reportlog) {
        $readers = $reportlog->get_readers(true);
        if (empty($readers)) {
            $readers = array(get_string('nologreaderenabled', 'report_advancedlog'));
        }
        ...
        $select->set_label(get_string('selectlogreader', 'report_advancedlog'));
     ...
     * @param report_advancedlog_renderable $reportlog log report.
     */
    public function report_selector_form(report_advancedlog_renderable $reportlog) {
        ...
        if (!empty($courses) && $reportlog->showcourses) {
            ...
            if (has_capability('report/advancedlog:view', $sitecontext)) {
                $a = new stdClass();
                $a->url = new moodle_url('/report/advancedlog/index.php', array('chooselog' => 0,
        ...
        echo html_writer::label(get_string('origin', 'report_advancedlog'), 'menuorigin', false, array('class' => 'accesshide'));
        ...
        if (!empty($readers)) {
            if (count($readers) == 1) {
                ...
                echo html_writer::label(get_string('selectlogreader', 'report_advancedlog'), 'menureader', false,
