 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class report_advancedlog_renderable implements renderable {
    ...
    public function get_activities_list() {
        ...
        if ($this->course->id == SITEID && has_capability('report/advancedlog:view', $sitecontext)) {
    ...
    public function get_course_list() {
        ...
        if (has_capability('report/advancedlog:view', $sitecontext) && $this->showcourses) {
    ...
    public function get_origin_options() {
        $ret = array();
        $ret[''] = get_string('allsources', 'report_advancedlog');
        $ret['cli'] = get_string('cli', 'report_advancedlog');
        $ret['restore'] = get_string('restore', 'report_advancedlog');
        $ret['web'] = get_string('web', 'report_advancedlog');
        $ret['ws'] = get_string('ws', 'report_advancedlog');
        $ret['---'] = get_string('other', 'report_advancedlog');
        return $ret;
    }
    ...
    public function setup_table() {
        ...
        $this->tablelog = new report_advancedlog_table_log('report_advancedlog', $filter);
